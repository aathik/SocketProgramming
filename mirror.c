...
#define ATR_PORT_NUM 7777

int atr_total_num_of_clients = 0, atr_num_of_arguments = 0;
...
void atr_tarfgetz(int atr_socket)
{
	...
	sprintf(atr_result, "find %s -type f -name '*.*' -size +%dc -size -%dc -print0 ", atr_home_path, atoi(atr_commands[1]), atoi(atr_commands[2]));
	...
	if (atr_proc_id == 0)
	{
		...
		while (1)
		{
			...
			if (strcmp(atr_msg_buffer, "quit") == 0)
			{
				...
				printf("Server disconnected from client %s:%d\n", inet_ntoa(atr_client_addr.sin_addr), ntohs(atr_client_addr.sin_port));
				close(atr_serverSide_socket);
				exit(0);
				break;
			}
...
int main()
{
	...
	while (1)
	{
		...
		printf("Client No-%d : Connection Established From %s:%d\n", atr_total_num_of_clients + 1, inet_ntoa(atr_client_addr.sin_addr), ntohs(atr_client_addr.sin_port));
